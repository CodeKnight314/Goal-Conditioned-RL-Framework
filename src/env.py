...
class PandasEnv:
        ...
        if self.use_wandb:
            wandb.init(
                ...
                tags=[
                    env_id,
                    agent_type,
                    "HER" if self.agent_config.buffer_type == "HER" else "Standard",
                ],
            )
    ...
    def log(self):
        ...
        logger.info(
            f"- Total Expected Gradient Steps: {round(self.config.max_frames/(self.num_envs)) * self.gradient_step}"
        )
        logger.info(
            f"- Total Expected Samples Read: {round(self.config.max_frames/(self.num_envs)) * self.gradient_step * self.config.agent.batch_size}"
        )
    ...
    def _process_step(self, state, actions, next_obs_raw, rewards, dones):
        ...
        obs_np = self.agent.normalize_state_batch(
            state["observation"], state["desired_goal"], normalize=self.normalize
        )
        ...
        for i in range(self.num_envs):
            self._push_to_buffer(
                i, obs_batch, actions, rewards, next_obs_batch, dones, next_obs_raw
            )

    def _push_to_buffer(
        self, i, obs_batch, actions, rewards, next_obs_batch, dones, state
    ):
        self.agent.push(
            obs_batch[i].detach(),
            actions[i],
    ...
    def train(self, path: str):
        ...
        while total_frames < self.config.max_frames:
            state_input = self.agent.normalize_state_batch(
                state["observation"], state["desired_goal"], self.normalize
            )
            ...
            for i in range(self.num_envs):
                ...
                if total_frames % self.save_freq == 0:
                    ...
                    if hasattr(self.agent.buffer, "obs_normalizer"):
                        self.agent.buffer.obs_normalizer.save(
                            os.path.join(path, "checkpoint", "obs.yaml")
                        )
                    if hasattr(self.agent.buffer, "dg_normalizer"):
                        self.agent.buffer.dg_normalizer.save(
                            os.path.join(path, "checkpoint", "dg.yaml")
                        )
                    if self.verbose:
                ...
                if total_frames % self.video_freq == 0:
                    ...
                    if self.verbose:
                        logger.info(
                            f"Video saved at {os.path.join(path, 'checkpoint')}"
                        )
            ...
            if self.agent.is_buffer_filled():
                for i in range(self.gradient_step):
                    ...
                    if len(info) == 9:  # SAC with actor update (includes alpha_loss)
                    ...
                    elif len(info) == 6:  # DDPG with actor update
                        q1_loss, ac_loss, td_error, q_value, critic_1_grad, ac_grad = (
                            info
                        )
                        self.history["ac_loss"].append(ac_loss)
                        self.history["actor_grad"].append(ac_grad)
                        self.history["alpha_loss"].append(0.0)
                        q2_loss, critic_2_grad = 0.0, 0.0
                    elif len(info) == 4:  # DDPG without actor update
                        q1_loss, td_error, q_value, critic_1_grad = info
                        q2_loss, critic_2_grad = 0.0, 0.0
                        ...
                        self.history["alpha_loss"].append(
                            0.0
                        )  # Default when no actor update
            ...
            if len(self.history["reward"]) >= self.config.window_size:
                ...
                if recent_reward_avg > self.best_reward:
                    ...
                    if self.verbose:
                        logger.info(
                            f"New best model saved! Average reward: {recent_reward_avg:.2f}"
                        )

            state = next_obs_raw

            self.history["reward_history"].append(
                np.mean(self.history["reward"])
                if len(self.history["reward"]) > 0
                else 0.0
            )
            self.history["ac_loss_history"].append(
                np.mean(self.history["ac_loss"])
                if len(self.history["ac_loss"]) > 0
                else 0.0
            )
            self.history["q1_loss_history"].append(
                np.mean(self.history["q1_loss"])
                if len(self.history["q1_loss"]) > 0
                else 0.0
            )
            self.history["q2_loss_history"].append(
                np.mean(self.history["q2_loss"])
                if len(self.history["q2_loss"]) > 0
                else 0.0
            )
            self.history["td_error_history"].append(
                np.mean(self.history["td_error"])
                if len(self.history["td_error"]) > 0
                else 0.0
            )
            self.history["q_value_history"].append(
                np.mean(self.history["q_value"])
                if len(self.history["q_value"]) > 0
                else 0.0
            )
            self.history["success_rate_history"].append(
                np.mean(self.history["success_rate"])
                if len(self.history["success_rate"]) > 0
                else 0.0
            )
            self.history["critic_1_grad_history"].append(
                np.mean(self.history["critic_1_grad"])
                if len(self.history["critic_1_grad"]) > 0
                else 0.0
            )
            self.history["critic_2_grad_history"].append(
                np.mean(self.history["critic_2_grad"])
                if len(self.history["critic_2_grad"]) > 0
                else 0.0
            )
            self.history["actor_grad_history"].append(
                np.mean(self.history["actor_grad"])
                if len(self.history["actor_grad"]) > 0
                else 0.0
            )
            self.history["alpha_loss_history"].append(
                np.mean(self.history["alpha_loss"])
                if len(self.history["alpha_loss"]) > 0
                else 0.0
            )

            if self.use_wandb and total_frames % 1000 == 0:
                best_reward_window = (
                    max(self.history["reward"])
                    if len(self.history["reward"]) > 0
                    else 0.0
                )
                log_dict = {
        ...
        final_success_rate = (
            self.history["success_rate_history"][-1]
            if self.history["success_rate_history"]
            else 0.0
        )
...
class PandasHEREnv(PandasEnv):
    def __init__(
        ...
        super().__init__(
            env_id, seed, config, num_envs, weights, verbose, use_wandb, agent_type
        )

        her_config = load_her_config(config)
        self.agent.buffer.compute_reward = gym.make(
            self.env_id
        ).unwrapped.compute_reward
        ...
        assert isinstance(
            self.agent.buffer, HERBuffer
        ), "[ERROR] Agent Buffer is not of HERBuffer type. Change in configs to HER"
    ...
    def log(self):
        ...
        logger.info(f"- Action Space: {self.ac_dim}")
        logger.info(
            f"- Total Expected Gradient Steps: {round(self.max_epoch * self.max_cycle * self.gradient_step)}"
        )
        logger.info(
            f"- Total Expected Episodes Read: {round(self.max_epoch * self.max_cycle * self.max_episode)}"
        )
        logger.info(f"Environment initialized with seed: {self.seed}")
    ...
    def _push_to_buffer(
        ...
        self.agent.push_her(
            ...
            rewards[i],
            dones[i],
            self.agent.normalize_goal(state["desired_goal"][i], self.normalize),
            self.agent.normalize_goal(state["achieved_goal"][i], self.normalize),
        )
    ...
    def train(self, path: str):
        ...
        for epoch in tqdm(range(1, self.max_epoch + 1), desc="Epoch", position=0):
            for cycle in tqdm(
                ...
                for i in range(self.gradient_step):
                    ...
                    if self.agent_type == "SAC" or self.agent_type == "TQC":
                        if len(info) == 9:
                            (
                                q1_loss,
                                q2_loss,
                ...
                if self.use_wandb:
                    ...
                    # Base metrics common to all agents
                    log_dict = {
                        "reward": self.history["reward_history"][-1],
                        "actor_loss": self.history["ac_loss_history"][-1],
                        "td_error": self.history["td_error_history"][-1],
                    ...
                    if self.agent_type == "SAC" or self.agent_type == "TQC":
                        log_dict.update(
                            {
                                "q1_loss": self.history["q1_loss_history"][-1],
                                "q2_loss": self.history["q2_loss_history"][-1],
                                "critic_1_grad": self.history["critic_1_grad_history"][
                                    -1
                                ],
                                "critic_2_grad": self.history["critic_2_grad_history"][
                                    -1
                                ],
                        ...
                        log_dict.update(
                            {
                                "critic_loss": self.history["q1_loss_history"][
                                    -1
                                ],  # For DDPG, q1_loss is actually critic_loss
                                "critic_grad": self.history["critic_1_grad_history"][
                                    -1
                                ],  # For DDPG, critic_1_grad is actually critic_grad
                            }
                        )
            ...
            if epoch % self.reset_freq == 0:
                ...
                if self.verbose:
                    logger.info(
                        f"Resetting TD3 Network across both Actor and Critic Networks"
                    )
        ...
        if hasattr(self.agent.buffer, "dg_normalizer"):
            self.agent.buffer.dg_normalizer.save(os.path.join(path, "dg.yaml"))
