...
import cv2
import wandb
from src.utils import load_config, set_seed, load_her_config, RunningNormalizer
...
class PandasEnv(): 
    ...
    def _process_step(self, state, actions, next_obs_raw, rewards, dones):
        self.agent.update_normalizers(
            ...
            [state["desired_goal"], next_obs_raw["desired_goal"]]
        )
    ...
    def warmup(self, warmup_steps: int = 800000):
    ...
    def train(self, path: str):
        ...
        while total_frames < self.config.max_frames:
            ...
            if self.agent.is_buffer_filled(): 
                ...
                self.agent.update_target_network(hard_update=False, tau=0.05)
    ...
    def test(self, path: str, num_episodes: int, smooth: bool = True, alpha: float = 0.65):
        ...
        for i in range(num_episodes):
            ...
            while not done:
                ...
                action = self.agent.select_action(normalized_state, eval_action=True)
            ...
            for cycle in tqdm(range(1, self.max_cycle + 1), desc="Cycle", position=1, leave=False):                
                ...
                if self.use_wandb:
                    ...
                    wandb.log({
                        ...
                        "critic_2_grad": self.history["critic_2_grad_history"][-1],
                        "actor_grad": self.history["actor_grad_history"][-1]
                    }, step=(epoch-1) * self.max_cycle + cycle)    
